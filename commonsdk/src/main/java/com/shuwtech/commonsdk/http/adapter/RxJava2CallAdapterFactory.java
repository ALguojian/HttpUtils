...
public final class RxJava2CallAdapterFactory extends CallAdapter.Factory {
    ...
    public CallAdapter<?, ?> get(Type returnType, Annotation[] annotations, Retrofit retrofit) {
        ...
        if (rawObservableType == Response.class) {
            ...
            responseType = getParameterUpperBound(0, (ParameterizedType) observableType);
            isResult = true;
        } else if (rawObservableType == BaseResponse.class) {
            if (!(observableType instanceof ParameterizedType)) {
        ...
        if (isBaseReponse) {
            return new BaseResponseCallAdapter(responseType, scheduler, isAsync, isFlowable, false);
        } else {
            return new RxJava2CallAdapter<>(responseType, scheduler, isAsync, isResult, isBody, isFlowable, false, false, false);
        }
    }
}
