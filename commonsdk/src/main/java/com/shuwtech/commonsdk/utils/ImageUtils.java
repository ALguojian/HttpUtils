...
public class ImageUtils {
    ...
    public static Bitmap imageZoom(Context context, Bitmap bitMap) {
        ...
        int screenHeight = PixelUtils.getScreenHeight(context);
    ...
    public static byte[] compressImage(Bitmap bitmap, double size) {
        //图片允许最大空间   单位：KB
    ...
    public static Bitmap compressWXImage(Bitmap image) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        image.compress(Bitmap.CompressFormat.JPEG, 100, baos);// 质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中
        int options = 100;
        //thumbit.getRowBytes() * thumbit.getHeight()
    ...
    public static Bitmap scaleBitmap(Bitmap bitmap, int width, int heigth) {
        int w = bitmap.getWidth();
        int h = bitmap.getHeight();
        Matrix matrix = new Matrix();
    ...
    public static void add2Gallery(String imgPath) {
        MediaScannerConnection.scanFile(CommonBaseApp.getApp(),
            new String[]{imgPath}, null, null);
    }
}
